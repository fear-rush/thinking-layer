...
class CleanCorpusBuildTest(unittest.TestCase):
    def write_raw(
        self,
        directory: Path,
        file_name: str,
    ...
    def test_rebuilds_clean_outputs_and_reports_ocr_coverage(self) -> None:
        with TemporaryDirectory(dir=ROOT) as temporary:
            ...
            self.assertEqual(result.lifecycle_relation_count, 1)
            self.assertEqual(result.skipped_ocr_file_ids, ("fixture-scan-only",))
            self.assertFalse((output_dir / "stale.txt").exists())
            manifest = json.loads(result.manifest_path.read_text(encoding="utf-8"))
            self.assertEqual(manifest["eligible_document_count"], 2)
            self.assertEqual(manifest["skipped_ocr_file_ids"], ["fixture-scan-only"])
            self.assertTrue((output_dir / "legal_nodes.ndjson").exists())
            self.assertTrue((output_dir / "contextual_units.ndjson").exists())
