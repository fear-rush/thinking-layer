...
from .search import execute_query_plan
...
def evidence_confidence(items: list[dict[str, Any]], expected_issuers: list[str | None], plan: dict[str, Any], query: str) -> dict[str, Any]:
    ...
    article_rate = sum(1 for item in top if item["has_article"]) / len(top)
    ...
    if not plan.get("topics") and not non_generic_entities and matched_phrase_count == 0 and (
...
def build_evidence_pack(query: str, max_searches: int, limit: int, per_document_limit: int) -> dict[str, Any]:
    ...
    for item in items:
        ...
        if len(grouped[key]) < per_document_limit:
    ...
    confidence = evidence_confidence(items, expected_issuers, plan, query)
    pack = {
        "query": query,
        "plan": plan,
